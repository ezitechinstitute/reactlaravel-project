import SEO from '../components/SEO'
import { Link } from 'react-router-dom'

export default function BlogDetails() {
  return (
    <>
      <SEO title="Blog Details" url="/blog-details" />
<main className="bg-background-5">
      
<section className="bg-background-6 relative overflow-hidden pt-36 pb-20 md:pt-39 md:pb-30 lg:pb-44">
  <div className="main-container relative z-10">
    <div className="space-y-9">
      <h2
        data-opai-animate
        data-delay="0.1"
        data-instant
        className="font-instrument-serif text-is-heading-4 md:text-is-heading-3 lg:text-is-heading-2 text-white/90"
      >
        How AI is transforming modern business operations
      </h2>

      
      <div data-opai-animate data-delay="0.2" data-instant className="space-y-[14px]">
        <div className="flex items-center gap-x-3">
          <figure className="size-12 overflow-hidden rounded-full">
            <img
              src="/images/opai-avatar-img-111.png"
              alt="avatar"
              className="size-full object-cover"
            />
          </figure>

          <div className="space-y-[1.5px]">
            <h4 className="font-inter-tight text-tagline-3 text-white">Noah Brown</h4>
            <p
              className="font-inter-tight text-tagline-4 flex items-center justify-center gap-x-[5px] font-normal text-white/60"
            >
              <span>30 November 2025</span>
              <span className="inline-block size-1.5 rounded-full bg-white/60"></span>
              <span> 5 min read</span>
            </p>
          </div>
        </div>

        <div data-opai-animate data-delay="0.3" data-instant>
          <ul className="flex items-center gap-3">
  <li>
    <a href="#" className="inline-block transition-transform duration-500 hover:-translate-y-1">
      <span className="sr-only">Facebook</span>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="20"
        height="20"
        viewBox="0 0 20 20"
        fill="none"
      >
        <path
          d="M10 19C14.9706 19 19 14.9706 19 10C19 5.02944 14.9706 1 10 1C5.02944 1 1 5.02944 1 10C1 14.9706 5.02944 19 10 19Z"
          stroke="#CED7DE"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M13.75 6.25H12.25C11.6533 6.25 11.081 6.48705 10.659 6.90901C10.2371 7.33097 10 7.90326 10 8.5V19"
          stroke="#CED7DE"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path d="M7 11.5H13" stroke="#CED7DE" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    </a>
  </li>
  <li>
    <a href="#" className="inline-block transition-transform duration-500 hover:-translate-y-1">
      <span className="sr-only">Instagram</span>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="20"
        height="20"
        viewBox="0 0 20 20"
        fill="none"
      >
        <path
          d="M10 13.75C12.0711 13.75 13.75 12.0711 13.75 10C13.75 7.92893 12.0711 6.25 10 6.25C7.92893 6.25 6.25 7.92893 6.25 10C6.25 12.0711 7.92893 13.75 10 13.75Z"
          stroke="#CED7DE"
          strokeMiterlimit="10"
        />
        <path
          d="M14.125 1.375H5.875C3.38972 1.375 1.375 3.38972 1.375 5.875V14.125C1.375 16.6103 3.38972 18.625 5.875 18.625H14.125C16.6103 18.625 18.625 16.6103 18.625 14.125V5.875C18.625 3.38972 16.6103 1.375 14.125 1.375Z"
          stroke="#CED7DE"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M14.875 6.25C15.4963 6.25 16 5.74632 16 5.125C16 4.50368 15.4963 4 14.875 4C14.2537 4 13.75 4.50368 13.75 5.125C13.75 5.74632 14.2537 6.25 14.875 6.25Z"
          fill="#CED7DE"
        />
      </svg>
    </a>
  </li>
  <li>
    <a href="#" className="inline-block transition-transform duration-500 hover:-translate-y-1">
      <span className="sr-only">X</span>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="17"
        height="17"
        viewBox="0 0 17 17"
        fill="none"
      >
        <path
          d="M15.2955 1L9.19287 7.29329M7.11364 9.4375L0.75 16M0.75 1L12.1136 16H15.75L4.38636 1H0.75Z"
          stroke="#CED7DE"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
    </a>
  </li>
  <li>
    <a href="#" className="inline-block transition-transform duration-500 hover:-translate-y-1">
      <span className="sr-only">LinkedIn</span>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="18"
        height="18"
        viewBox="0 0 18 18"
        fill="none"
      >
        <path
          d="M16.5 0.75H1.5C1.08579 0.75 0.75 1.08579 0.75 1.5V16.5C0.75 16.9142 1.08579 17.25 1.5 17.25H16.5C16.9142 17.25 17.25 16.9142 17.25 16.5V1.5C17.25 1.08579 16.9142 0.75 16.5 0.75Z"
          stroke="#CED7DE"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path d="M8.25 7.5V13.5" stroke="#CED7DE" strokeLinecap="round" strokeLinejoin="round" />
        <path d="M5.25 7.5V13.5" stroke="#CED7DE" strokeLinecap="round" strokeLinejoin="round" />
        <path
          d="M8.25 10.125C8.25 9.42881 8.52656 8.76113 9.01884 8.26884C9.51113 7.77656 10.1788 7.5 10.875 7.5C11.5712 7.5 12.2389 7.77656 12.7312 8.26884C13.2234 8.76113 13.5 9.42881 13.5 10.125V13.5"
          stroke="#CED7DE"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M5.25 5.625C5.87132 5.625 6.375 5.12132 6.375 4.5C6.375 3.87868 5.87132 3.375 5.25 3.375C4.62868 3.375 4.125 3.87868 4.125 4.5C4.125 5.12132 4.62868 5.625 5.25 5.625Z"
          fill="#CED7DE"
        />
      </svg>
    </a>
  </li>
  <li>
    <a href="#" className="inline-block transition-transform duration-500 hover:-translate-y-1">
      <span className="sr-only">YouTube</span>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="24"
        height="24"
        viewBox="0 0 24 24"
        fill="none"
      >
        <path
          d="M22.5406 6.42C22.4218 5.94541 22.1799 5.51057 21.8392 5.15941C21.4986 4.80824 21.0713 4.55318 20.6006 4.42C18.8806 4 12.0006 4 12.0006 4C12.0006 4 5.12057 4 3.40057 4.46C2.92982 4.59318 2.50255 4.84824 2.16192 5.19941C1.82129 5.55057 1.57936 5.98541 1.46057 6.46C1.14579 8.20556 0.991808 9.97631 1.00057 11.75C0.989351 13.537 1.14334 15.3213 1.46057 17.08C1.59153 17.5398 1.83888 17.9581 2.17872 18.2945C2.51855 18.6308 2.93939 18.8738 3.40057 19C5.12057 19.46 12.0006 19.46 12.0006 19.46C12.0006 19.46 18.8806 19.46 20.6006 19C21.0713 18.8668 21.4986 18.6118 21.8392 18.2606C22.1799 17.9094 22.4218 17.4746 22.5406 17C22.8529 15.2676 23.0069 13.5103 23.0006 11.75C23.0118 9.96295 22.8578 8.1787 22.5406 6.42V6.42Z"
          stroke="#CED7DE"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M9.75 15.0166L15.5 11.7466L9.75 8.47656V15.0166Z"
          stroke="#CED7DE"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
    </a>
  </li>
</ul>

        </div>
      </div>

      
      <figure
        data-opai-animate
        data-delay="0.4"
        data-instant
        className="h-[450px] w-full overflow-hidden rounded-[10px] md:h-[600px]"
      >
        <img src="/images/opai-img-30.png" alt="blog" className="size-full object-cover" />
      </figure>

      <div className="spcae-y-3">
        <h3
          data-opai-animate
          data-delay="0.1"
          className="font-instrument-serif text-is-heading-4 text-background-13/90 font-normal"
        >
          Introduction
        </h3>
        <p
          data-opai-animate
          data-delay="0.2"
          className="font-inter-tight text-tagline-2 text-background-13/60 font-normal"
        >
          Artificial Intelligence is revolutionizing the way companies function by optimizing
          processes, improving customer interactions, and speeding up innovation. In this
          comprehensive blog post, we delve into the various ways AI is changing the landscape of
          contemporary business. From automating routine tasks to providing insights that drive
          strategic decisions, AI is not just a trend but a pivotal force that every organization
          should harness. Join us as we explore the immense potential of AI and discuss why now is
          the perfect time to integrate these technologies into your business strategy.
        </p>
      </div>
    </div>
  </div>

  <div>
  <div
    className="bg-opai-purple absolute bottom-[9%] left-1/2 z-1 mx-auto h-[304px] w-[70%] -translate-x-1/2 rounded-[420px] blur-[142px]"
  ></div>

  <div
    className="bg-opai-purple absolute bottom-[6%] left-1/2 z-2 h-[345px] w-[110%] -translate-x-1/2 rounded-[420px] mix-blend-color-dodge blur-[162px]"
  ></div>

  <div
    className="bg-background-7 absolute bottom-0 left-1/2 z-3 h-[297px] w-[110%] -translate-x-1/2 rounded-[120px] blur-[107px]"
  ></div>
  <div
    className="bg-background-7 absolute -bottom-10 left-1/2 z-4 h-[175px] w-[110%] -translate-x-1/2 rounded-[80px] blur-[28px]"
  ></div>
  <div
    className="bg-background-7 absolute -bottom-5 left-1/2 z-5 h-[55px] w-[110%] -translate-x-1/2 rounded-[80px] blur-[28px]"
  ></div>

  <div className="bg-background-7 absolute -bottom-4 left-0 z-6 h-[55px] w-40 blur-[10px]"></div>
  <div className="bg-background-7 absolute right-0 -bottom-4 z-6 h-[55px] w-40 blur-[10px]"></div>
</div>

</section>

      <section className="bg-background-7 pb-20 md:pb-30 lg:pb-44">
  <div className="main-container">
    <div className="space-y-8">
      <div className="flex flex-col items-center justify-between gap-6 md:flex-row">
        <figure
          data-opai-animate
          data-delay="0.1"
          className="h-[400px] w-full overflow-hidden rounded-xl lg:h-[550px]"
        >
          <img src="/images/opai-img-44.png" alt="blog" className="size-full object-cover" />
        </figure>
        <figure
          data-opai-animate
          data-delay="0.2"
          className="h-[400px] w-full overflow-hidden rounded-xl lg:h-[550px]"
        >
          <img src="/images/opai-img-45.png" alt="blog" className="size-full object-cover" />
        </figure>
      </div>

      <div data-opai-animate data-delay="0.3" className="blog-details-markdown">
        <h3>AI and the evolution of business operations</h3>
        <p>
          Modern businesses are under constant pressure to move faster, reduce costs, and make
          better decisions. Artificial Intelligence is no longer experimental—it has become a core
          driver of how companies operate, compete, and scale in today’s digital economy.
        </p>

        <h3>AI in workflow automation</h3>
        <p>
          Repetitive and manual tasks slow teams down and increase the risk of human error.
          AI-powered workflow automation tools now handle scheduling, data processing, and task
          prioritization with minimal human input, allowing teams to focus on meaningful work.
        </p>

        <ul>
          <li>Reduce time spent on repetitive administrative tasks.</li>
          <li>Improve operational speed and consistency.</li>
          <li>Enable employees to focus on strategic initiatives.</li>
        </ul>

        <blockquote>
          “AI doesn’t replace human effort—it amplifies it. The real advantage comes from freeing
          people to work on what matters most.”
        </blockquote>

        <h3>Smarter decision-making with AI</h3>
        <p>
          AI systems analyze large volumes of data in real time, uncovering patterns that are
          difficult for humans to detect. Businesses use these insights to make faster, more
          accurate decisions across finance, marketing, operations, and product development.
        </p>

        <ul>
          <li>Predict trends using data-driven forecasting.</li>
          <li>Identify risks and opportunities earlier.</li>
          <li>Improve planning accuracy across departments.</li>
        </ul>

        <h3>AI-powered customer experience</h3>
        <p>
          From chatbots to personalized recommendations, AI is transforming how businesses interact
          with customers. Intelligent systems provide instant support, tailored content, and
          consistent experiences across multiple channels.
        </p>

        <ul>
          <li>Resolve common customer queries instantly.</li>
          <li>Deliver personalized experiences at scale.</li>
          <li>Maintain 24/7 customer support availability.</li>
        </ul>

        <h3>Operational efficiency and cost optimization</h3>
        <p>
          AI helps organizations optimize resources by identifying inefficiencies and automating
          cost-heavy processes. Predictive maintenance, inventory optimization, and demand
          forecasting are just a few areas where AI delivers measurable savings.
        </p>

        <ul>
          <li>Lower operational costs through automation.</li>
          <li>Minimize downtime with predictive insights.</li>
          <li>Optimize supply chains and resource allocation.</li>
        </ul>

        <h3>Conclusion</h3>
        <p>
          AI is no longer a future consideration—it is reshaping how modern businesses operate
          today. Organizations that embrace AI-driven automation, smarter decision-making, and
          enhanced customer experiences gain a clear competitive advantage. The question is no
          longer whether to adopt AI, but how quickly it can be integrated into everyday operations.
        </p>
      </div>

      

      <div
        data-opai-animate
        data-delay="0.1"
        className="divide-background-13/10 mt-[70px] space-y-6 divide-y"
      >
        <div className="space-y-4 pb-6">
          <p
            data-opai-animate
            data-delay="0.1"
            className="font-inter-tight text-tagline-2 text-background-13/90 font-medium"
          >
            Share this post
          </p>
          <div data-opai-animate data-delay="0.2">
            <ul className="flex items-center gap-3">
  <li>
    <a href="#" className="inline-block transition-transform duration-500 hover:-translate-y-1">
      <span className="sr-only">Facebook</span>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="20"
        height="20"
        viewBox="0 0 20 20"
        fill="none"
      >
        <path
          d="M10 19C14.9706 19 19 14.9706 19 10C19 5.02944 14.9706 1 10 1C5.02944 1 1 5.02944 1 10C1 14.9706 5.02944 19 10 19Z"
          stroke="#0D1017"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M13.75 6.25H12.25C11.6533 6.25 11.081 6.48705 10.659 6.90901C10.2371 7.33097 10 7.90326 10 8.5V19"
          stroke="#0D1017"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path d="M7 11.5H13" stroke="#0D1017" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    </a>
  </li>
  <li>
    <a href="#" className="inline-block transition-transform duration-500 hover:-translate-y-1">
      <span className="sr-only">Instagram</span>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="20"
        height="20"
        viewBox="0 0 20 20"
        fill="none"
      >
        <path
          d="M10 13.75C12.0711 13.75 13.75 12.0711 13.75 10C13.75 7.92893 12.0711 6.25 10 6.25C7.92893 6.25 6.25 7.92893 6.25 10C6.25 12.0711 7.92893 13.75 10 13.75Z"
          stroke="#0D1017"
          strokeMiterlimit="10"
        />
        <path
          d="M14.125 1.375H5.875C3.38972 1.375 1.375 3.38972 1.375 5.875V14.125C1.375 16.6103 3.38972 18.625 5.875 18.625H14.125C16.6103 18.625 18.625 16.6103 18.625 14.125V5.875C18.625 3.38972 16.6103 1.375 14.125 1.375Z"
          stroke="#0D1017"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M14.875 6.25C15.4963 6.25 16 5.74632 16 5.125C16 4.50368 15.4963 4 14.875 4C14.2537 4 13.75 4.50368 13.75 5.125C13.75 5.74632 14.2537 6.25 14.875 6.25Z"
          fill="#0D1017"
        />
      </svg>
    </a>
  </li>
  <li>
    <a href="#" className="inline-block transition-transform duration-500 hover:-translate-y-1">
      <span className="sr-only">X</span>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="17"
        height="17"
        viewBox="0 0 17 17"
        fill="none"
      >
        <path
          d="M15.2955 1L9.19287 7.29329M7.11364 9.4375L0.75 16M0.75 1L12.1136 16H15.75L4.38636 1H0.75Z"
          stroke="#0D1017"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
    </a>
  </li>
  <li>
    <a href="#" className="inline-block transition-transform duration-500 hover:-translate-y-1">
      <span className="sr-only">LinkedIn</span>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="18"
        height="18"
        viewBox="0 0 18 18"
        fill="none"
      >
        <path
          d="M16.5 0.75H1.5C1.08579 0.75 0.75 1.08579 0.75 1.5V16.5C0.75 16.9142 1.08579 17.25 1.5 17.25H16.5C16.9142 17.25 17.25 16.9142 17.25 16.5V1.5C17.25 1.08579 16.9142 0.75 16.5 0.75Z"
          stroke="#0D1017"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path d="M8.25 7.5V13.5" stroke="#0D1017" strokeLinecap="round" strokeLinejoin="round" />
        <path d="M5.25 7.5V13.5" stroke="#0D1017" strokeLinecap="round" strokeLinejoin="round" />
        <path
          d="M8.25 10.125C8.25 9.42881 8.52656 8.76113 9.01884 8.26884C9.51113 7.77656 10.1788 7.5 10.875 7.5C11.5712 7.5 12.2389 7.77656 12.7312 8.26884C13.2234 8.76113 13.5 9.42881 13.5 10.125V13.5"
          stroke="#0D1017"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M5.25 5.625C5.87132 5.625 6.375 5.12132 6.375 4.5C6.375 3.87868 5.87132 3.375 5.25 3.375C4.62868 3.375 4.125 3.87868 4.125 4.5C4.125 5.12132 4.62868 5.625 5.25 5.625Z"
          fill="#0D1017"
        />
      </svg>
    </a>
  </li>
  <li>
    <a href="#" className="inline-block transition-transform duration-500 hover:-translate-y-1">
      <span className="sr-only">YouTube</span>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="24"
        height="24"
        viewBox="0 0 24 24"
        fill="none"
      >
        <path
          d="M22.5406 6.42C22.4218 5.94541 22.1799 5.51057 21.8392 5.15941C21.4986 4.80824 21.0713 4.55318 20.6006 4.42C18.8806 4 12.0006 4 12.0006 4C12.0006 4 5.12057 4 3.40057 4.46C2.92982 4.59318 2.50255 4.84824 2.16192 5.19941C1.82129 5.55057 1.57936 5.98541 1.46057 6.46C1.14579 8.20556 0.991808 9.97631 1.00057 11.75C0.989351 13.537 1.14334 15.3213 1.46057 17.08C1.59153 17.5398 1.83888 17.9581 2.17872 18.2945C2.51855 18.6308 2.93939 18.8738 3.40057 19C5.12057 19.46 12.0006 19.46 12.0006 19.46C12.0006 19.46 18.8806 19.46 20.6006 19C21.0713 18.8668 21.4986 18.6118 21.8392 18.2606C22.1799 17.9094 22.4218 17.4746 22.5406 17C22.8529 15.2676 23.0069 13.5103 23.0006 11.75C23.0118 9.96295 22.8578 8.1787 22.5406 6.42V6.42Z"
          stroke="#0D1017"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M9.75 15.0166L15.5 11.7466L9.75 8.47656V15.0166Z"
          stroke="#0D1017"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
    </a>
  </li>
</ul>

          </div>
        </div>

        

        <div data-opai-animate data-delay="0.3" className="flex items-center gap-x-3">
          <figure className="size-12 overflow-hidden rounded-full">
            <img
              src="/images/opai-avatar-img-111.png"
              alt="avatar"
              className="size-full object-cover"
            />
          </figure>

          <div className="space-y-[1.5px]">
            <h4 className="font-inter-tight text-tagline-3 text-background-13/90">Noah Brown</h4>
            <p
              className="font-inter-tight text-tagline-4 text-background-13/60 flex items-center justify-center gap-x-[5px] font-normal"
            >
              <span>30 November 2025</span>
              <span className="bg-background-13/60 inline-block size-1 rounded-full"></span>
              <span> 5 min read</span>
            </p>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>

      

<section className="bg-background-7 pb-20 md:pb-30 lg:pb-44">
  <div className="main-container">
    <div className="space-y-14 md:space-y-[70px]">
      <div className="space-y-3">
        <h2
          data-opai-animate
          data-delay="0.1"
          className="font-instrument-serif text-is-heading-4 md:text-is-heading-3 lg:text-is-heading-2 text-background-13/90 font-normal"
        >
          Related articles
        </h2>
        <p
          data-opai-animate
          data-delay="0.2"
          className="text-tagline-2 text-background-13/60 font-inter-tight font-normal"
        >
          Discover more insights and strategies from our latest blog articles to help you stay ahead
          in AI-driven marketing and business optimization.
        </p>
      </div>

      
      <div
        className="flex items-start justify-center overflow-hidden max-lg:flex-wrap max-lg:gap-y-9 md:space-x-[30px]"
        aria-label="Featured blog posts"
      >
        
        <article
          className="group underline-hover-effect-black blog-card active-card relative min-h-[545px] space-y-4"
          data-opai-animate
          data-delay="0.4"
        >
          <a href="/blog-details" className="block">
            <figure className="h-[380px] w-full overflow-hidden rounded-lg">
              <img
                src="/images/opai-img-159.jpg"
                alt="Performance Marketing Playbook: How to Scale Paid Ads with AI"
                className="h-full w-full object-cover transition-all duration-500 ease-in-out group-hover:scale-105 group-hover:rotate-2"
              />
            </figure>
          </a>
          <div className="space-y-4 px-2">
            <p className="text-tagline-5 text-background-13/80 font-normal">28/6/2025</p>
            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                <span
                  className="bg-background-13/5 text-tagline-4 text-background-13/50 rounded-full px-3 py-1 font-normal"
                  >Ads</span
                >
                <span
                  className="bg-background-13/5 text-tagline-4 text-background-13/50 rounded-full px-3 py-1 font-normal"
                  >Performance</span
                >
              </div>
              <a href="/blog-details" className="blog-title-black">
                <h3 className="text-is-heading-5 text-background-13 font-normal">
                  Performance marketing playbook: scale paid ads with AI
                </h3>
              </a>
            </div>
          </div>
        </article>

        
        <article
          className="group underline-hover-effect-black blog-card relative min-h-[545px] space-y-4"
          data-opai-animate
          data-delay="0.5"
        >
          <a href="/blog-details" className="block">
            <figure className="h-[380px] w-full overflow-hidden rounded-lg">
              <img
                src="/images/opai-avatar-img-170.jpg"
                alt="Email Automation That Converts: AI Segmentation + Personalization"
                className="h-full w-full object-cover transition-all duration-500 ease-in-out group-hover:scale-105 group-hover:rotate-2"
              />
            </figure>
          </a>
          <div className="space-y-4 px-2">
            <p className="text-tagline-5 text-background-13/80 font-normal">28/6/2025</p>
            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                <span
                  className="bg-background-13/5 text-tagline-4 text-background-13/50 rounded-full px-3 py-1 font-normal"
                  >Email</span
                >
                <span
                  className="bg-background-13/5 text-tagline-4 text-background-13/50 rounded-full px-3 py-1 font-normal"
                  >Automation</span
                >
              </div>
              <a href="/blog-details" className="blog-title-black">
                <h3 className="text-is-heading-5 text-background-13 font-normal">
                  Email automation that converts segmentation
                </h3>
              </a>
            </div>
          </div>
        </article>

        
        <article
          className="group underline-hover-effect-black blog-card relative min-h-[545px] space-y-4"
          data-opai-animate
          data-delay="0.6"
        >
          <a href="/blog-details" className="block">
            <figure className="h-[380px] w-full overflow-hidden rounded-lg">
              <img
                src="/images/opai-avatar-img-171.jpg"
                alt="Landing Page CRO Checklist: Fix Leaks and Increase Conversions"
                className="h-full w-full object-cover transition-all duration-500 ease-in-out group-hover:scale-105 group-hover:rotate-2"
              />
            </figure>
          </a>
          <div className="space-y-4 px-2">
            <p className="text-tagline-5 text-background-13/80 font-normal">28/6/2025</p>
            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                <span
                  className="bg-background-13/5 text-tagline-4 text-background-13/50 rounded-full px-3 py-1 font-normal"
                  >CRO</span
                >
                <span
                  className="bg-background-13/5 text-tagline-4 text-background-13/50 rounded-full px-3 py-1 font-normal"
                  >Landing Page</span
                >
              </div>
              <a href="/blog-details" className="blog-title-black">
                <h3 className="text-is-heading-5 text-background-13 font-normal">
                  Landing page CRO checklist: fix leaks and lift conversions
                </h3>
              </a>
            </div>
          </div>
        </article>
      </div>
    </div>
  </div>
</section>

      

<section
  className="bg-background-6 !pt-20 md:!pt-30 lg:!pt-44 overflow-hidden py-14 md:py-20 lg:py-24 xl:py-28"
>
  <div className="main-container">
    <div className="space-y-14 pb-9">
      

      <div className="flex items-center justify-center gap-x-2">
        <img
          data-opai-avatar
          data-avatar-delay="0.1"
          className="inline-block size-14 rounded-full"
          src="/images/opai-avatar-img-05.png"
          alt="Marketing Expert 1"
        />
        <img
          data-opai-avatar
          data-avatar-delay="0.2"
          className="inline-block size-14 rounded-full"
          src="/images/opai-avatar-img-06.png"
          alt="Marketing Expert 2"
        />
        <img
          data-opai-avatar
          data-avatar-delay="0.3"
          className="inline-block size-14 rounded-full"
          src="/images/opai-avatar-img-07.png"
          alt="Marketing Expert 3"
        />
        <img
          data-opai-avatar
          data-avatar-delay="0.4"
          className="inline-block size-14 rounded-full"
          src="/images/opai-avatar-img-08.png"
          alt="Marketing Expert 4"
        />
      </div>
      <div className="space-y-3">
        <h2
          className="text-is-heading-3 lg:text-is-heading-2 font-instrument-serif text-center font-normal tracking-[-2.4px] text-white/90"
          data-opai-animate
          data-delay="0.1"
        >
          
          <span
            className="inline-flex flex-wrap items-start justify-center gap-x-1 sm:flex-nowrap md:items-center"
          >
            <span className="w-full sm:w-auto">Turn clicks</span>
            <span
              className="cta-content-image inline-block h-[52px] w-[88px] overflow-hidden rounded-[300px] align-middle"
            >
              <img
                src="/images/opai-img-24.png"
                alt=""
                aria-hidden="true"
                className="h-full w-full object-cover"
              />
            </span>
            <span className="w-full sm:w-auto">Into customers</span>
          </span>

          <br />

          
          <span className="inline-flex items-center justify-center gap-x-4">
            <span
              className="cta-content-image-2 inline-block size-12 -rotate-[20deg] overflow-hidden rounded-lg align-middle"
            >
              <img
                src="/images/opai-avatar-img-25.png"
                alt=""
                aria-hidden="true"
                className="h-full w-full object-cover"
              />
            </span>
            <span>the smarter way</span>
          </span>
        </h2>

        <p
          className="text-tagline-2 mx-auto w-full max-w-[320px] text-center text-white/60"
          data-opai-animate
          data-delay="0.2"
        >
          No guesswork. We use AI + proven marketing systems to scale ads, SEO,
          and conversion.
        </p>
      </div>

      <div className="flex justify-center" data-opai-animate data-delay="0.3">
        <a
  href="/contact"
  className="group bg-background-7 hover:border-stroke-3 {=$anchor-class} relative flex items-center justify-center gap-2 overflow-hidden rounded-xl border border-transparent px-6 py-[13px] transition-all duration-300 ease-in-out"
>
  <div className="h-full max-h-5 overflow-hidden">
    <span
      className="font-ibm-plex-mono text-background-13/90 text-tagline-2 block -translate-y-0.5 leading-[1.4] font-medium text-nowrap transition-transform duration-500 ease-in-out group-hover:-translate-y-[105%]"
    >
      Book a strategy call
    </span>
    <span
      className="font-ibm-plex-mono text-tagline-2 block leading-[1.4] font-medium text-nowrap transition-transform duration-500 ease-in-out group-hover:-translate-y-[105%]"
    >
      Book a strategy call
    </span>
  </div>

  <div className="relative mt-[1.5px] flex size-6 items-center justify-center overflow-hidden">
    
    <span
      className="bg-background-6 absolute size-[22px] translate-x-0 rounded-full transition-all duration-400 ease-in-out"
    >
      
      <span
        className="stroke-background-10 absolute size-[16px] translate-x-[2.5px] translate-y-0.5 scale-95 stroke-2 transition-all duration-400 ease-in-out group-hover:translate-x-6"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="16"
          height="16"
          viewBox="0 0 16 16"
          fill="none"
        >
          <path d="M6.75 13.5L11.25 9L6.75 4.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </span>
    </span>

    
    <span
      className="stroke-background-10 absolute size-[18px] -translate-x-6 stroke-2 transition-all duration-400 ease-in-out group-hover:translate-x-0"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="18"
        height="18"
        viewBox="0 0 18 18"
        fill="none"
      >
        <path d="M6.75 13.5L11.25 9L6.75 4.5" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    </span>
  </div>
</a>

      </div>
    </div>

    <div
      data-opai-animate
      data-delay="0.4"
      data-duration="3"
      data-direction="right"
      data-spring
    >
      <figure className="flex justify-end">
        <img src="/images/vector-01.svg" alt="CTA Image" />
      </figure>
    </div>
  </div>
</section>

    </main>
    </>
  )
}
